"""STAR PLATINUM（スタープラチナ）— Part 3 · Jotaro Kujo.

Stand: STAR PLATINUM
Ability: Supreme precision, speed, time stop (Star Platinum: The World)

As a JoJo persona: direct, efficient, no-nonsense task execution.
Uses ReAct loop with maximum precision and minimum wasted steps.
"""
...
class StarPlatinum(JoJoStand):
    """「やれやれだぜ…」"""

    stand_type = JoJoStandType.STAR_PLATINUM
    ...
    def build_system_prompt(self, tool_descriptions: str, stand_descriptions: str) -> str:
        ...
        return (
            ...
            "Be direct. Be precise. Do not waste steps.\n"
            "Use tools when needed. Cite your sources. Get it done.\n\n"
            "「オラオラオラ！」\n\n"
            f"## Available Tools\n{tool_descriptions}"
        )

    async def run(self, user_input: str, context: dict[str, Any]) -> dict[str, Any]:
        """Standard ReAct loop — precision execution."""
        ...
        return {"answer": "Star Platinum: max steps reached. やれやれだぜ…", "steps": max_steps, "tool_calls": tool_records}
