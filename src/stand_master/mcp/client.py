...
class MCPClientManager:
    ...
    async def connect_all(self, configs: dict[str, MCPServerConfig], *, max_retries: int = 3) -> None:
        for name, config in configs.items():
            await self._connect_one(name, config, max_retries=max_retries)

    async def _connect_one(self, name: str, config: MCPServerConfig, *, max_retries: int = 3) -> None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                self._connections[name] = _ServerConnection(
                    name=name, config=config, session=session, _context_stack=ctx,
                )
                ...
                last_exc = exc
                delay = 2 ** attempt
                logger.warning("Failed to connect to '%s' (attempt %d/%d): %s", name, attempt + 1, max_retries, exc)
                await asyncio.sleep(delay)

        logger.error("Could not connect to MCP server '%s' after %d attempts", name, max_retries)
        if last_exc:
            raise ConnectionError(f"Failed to connect to MCP server '{name}': {last_exc}") from last_exc

    async def list_tools(self, server_name: str | None = None) -> list[ToolInfo]:
        servers = [self._get(server_name)] if server_name else list(self._connections.values())
            ...
            try:
                ...
                for tool in result.tools:
                    all_tools.append(ToolInfo(
                        server_name=conn.name, name=tool.name,
