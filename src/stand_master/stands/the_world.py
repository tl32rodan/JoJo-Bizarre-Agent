...
Deep chain-of-thought reasoning pipeline (in-process).
Uses a dedicated reasoning model with higher step budget.
This is the only Stand that runs a full ReAct inner loop.
"""
...
class TheWorld(Stand):
    ...
    def __init__(self, llm: Any, tool_registry: Any | None = None, max_steps: int = 30) -> None:
        super().__init__()
        self._llm = llm
        ...
        try:
            ...
            for step in range(self._max_steps):
                ...
                if not raw_tool_calls:
                    ...
                    return self._succeed(answer, steps=step + 1, tool_calls=tool_calls_log)
                ...
                for tc in raw_tool_calls:
                    ...
                    messages.append({
                        "role": "tool", "content": str(result)[:4000],
                        "tool_call_id": tc.get("id", ""),
                    })
                    tool_calls_log.append({"name": tool_name, "arguments": tool_args, "result": str(result)[:500]})

            return self._succeed("THE WORLD: maximum reasoning steps reached.", steps=self._max_steps, tool_calls=tool_calls_log)
